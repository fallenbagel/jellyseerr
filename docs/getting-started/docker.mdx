...
#### Compose Setup Options
Jellyseerr supports multiple Compose files for different environments:

##### 🛠 Local Development
Use for contributing or debugging with live reload and local source mapping.
```bash
docker compose -f compose.local.yml up
```
- Uses `Dockerfile.local`
- Mounts local code and dependencies
- Defaults to SQLite

##### 📆 Production (SQLite)
Simple production deployment using default database.
```bash
docker compose up -d
```
- Uses image from `ghcr.io`
- Persists config to volume
- No additional setup required

##### 🛡 Production (PostgreSQL)
Recommended for scalable or multi-user production setups.
```bash
docker compose -f compose.yml -f compose.postgres.yml up -d
```
- Enables external Postgres service
- Persists data using named volume
- Requires proper DB credentials via environment variables

All files follow modern Compose spec and omit the deprecated `version:` key.
