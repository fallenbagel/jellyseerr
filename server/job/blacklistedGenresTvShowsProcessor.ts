...
class BlacklistedGenresTvShowsProcessor implements RunnableScanner<StatusBase> {
  ...
  public async run() {
    ...
    try {
      ...
      if (err instanceof AbortTransaction) {
        logger.info('Aborting job: Process Blacklisted TV Show Genres', {
          label: 'Jobs',
        });
      } else {
        throw err;
  ...
  private async createBlacklistEntries(em: EntityManager) {
    ...
    const blacklistedGenres = settings.main.blacklistedGenresTvShows || '';
    const blacklistedGenresArr = blacklistedGenres
      .split(',')
      .filter((g) => g.trim());
    const pageLimit = settings.main.blacklistedGenresTvShowsLimit || 50;
    ...
    const availableGenres = await tmdb.getTvGenres();
    ...
    for (const genreId of blacklistedGenresArr) {
      ...
      if (!genreDetails) {
        logger.warn('Skipping invalid genre in blacklisted TV show genres', {
          label: 'Blacklisted TV Show Genres Processor',
          genreId: genreId,
        });
      ...
      for (const sortBy of SortOptionsIterable) {
        for (let page = 1; page <= pageLimit; page++) {
          ...
          try {
            const response = await tmdb.getDiscoverTv({
              page,
              sortBy,
              genre: genreId,
            });
            ...
            logger.error(
              'Error processing genre in blacklisted TV show genres',
              {
                label: 'Blacklisted TV Show Genres Processor',
                genreId: genreId,
    ...
    if (invalidGenres.size > 0) {
      ...
      if (cleanedGenres !== blacklistedGenres) {
        ...
        logger.info('Cleaned up invalid TV show genres from settings', {
          label: 'Blacklisted TV Show Genres Processor',
  ...
  private async processResults(
    ...
    for (const entry of response.results) {
      ...
      if (blacklistEntry) {
        const currentGenres = blacklistEntry.blacklistedGenresTvShows;
        if (currentGenres && !currentGenres.includes(`,${genreId},`)) {
          await blacklistRepository.update(blacklistEntry.id, {
            blacklistedGenresTvShows: `${currentGenres}${genreId},`,
          });
        } else if (!currentGenres) {
          await blacklistRepository.update(blacklistEntry.id, {
            blacklistedGenresTvShows: `,${genreId},`,
          });
        }
      } else {
  ...
  private async cleanBlacklist(em: EntityManager) {
    ...
    const mediaToRemove = await mediaRepository
      ...
      .where(
        `blist.blacklistedGenresTvShows IS NOT NULL AND media.mediaType = 'tv'`
      )
      .getMany();
...
const blacklistedGenresTvShowsProcessor =
  new BlacklistedGenresTvShowsProcessor();

export default blacklistedGenresTvShowsProcessor;
